...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh start9os_yamls.sh check-health.sh
ifeq ($(ARCH),aarch64)
else
...
docker-images/aarch64.tar: Dockerfile docker_entrypoint.sh start9os_yamls.sh check-health.sh
ifeq ($(ARCH),x86_64)
else
	mkdir -p docker-images
